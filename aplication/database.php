...
class ClassPDO
{
	...
	public function __construct(
		$drive = "mysql",
		$host = "localhost",
		$database = "monkey_tracking",
		$username = "root",
		$password = "normandyDM29"
	)
	{
		$this->drive = $drive;
		$this->host = $host;
	...
	public function find($table, $query=NULL, $options=array())
	{
		...
		//Condición donde paso la columna a la que se le aplicará la función Suma SQL
		...
		switch ($query)
		{
			...
			//Caso que realiza la suma de la columna amount de la tabla transacciones
			case 'suma':
